...
After writing the deck, update `docs/ai/decks/index.md` to list all decks in
the directory. Read the existing index (or create it if missing), then rewrite
it with a table row for every `*.html` file in `docs/ai/decks/`, sorted by
date descending. Use the same format:
...
|------|------|-------------|
| YYYY-MM-DD | [Title](./filename.html) | One-line description |
```
